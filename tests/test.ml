...
let compress_stream ?(level = 3) data =
  ...
  in
  let state = Zstd.Compressor.State.create ~level () in
  let bs = Bstr.of_string data in
  Zstd.Compressor.State.feed ~push state bs 0 (Bstr.length bs) `Continue;
  Zstd.Compressor.State.finish ~push state;
  Buffer.contents buf
...
let decompress_stream ?size_limit compressed =
  ...
  let push bs pos len =
    Buffer.add_string buf (Bstr.sub ~off:pos ~len bs |> Bstr.to_string)
  in
  let state = Zstd.Decompressor.State.create ?size_limit () in
  let bs = Bstr.of_string compressed in
  Zstd.Decompressor.State.feed ~push state bs 0 (Bstr.length bs);
...
let test_string_roundtrip () =
  ...
  let compressed = Zstd.Compressor.compress_string ~level:3 data in
  let decompressed = Zstd.Decompressor.decompress_string compressed in
...
let test_bigstring_roundtrip () =
  ...
  let compressed = Zstd.Compressor.compress_bigstring ~level:3 data in
  let decompressed = Zstd.Decompressor.decompress_bigstring compressed in
...
let test_compression_levels () =
  ...
  for level = 1 to 19 do
    let compressed = Zstd.Compressor.compress_string ~level data in
    let decompressed = Zstd.Decompressor.decompress_string compressed in
...
let test_stream_to_simple () =
  (* NOTE: streaming compression does not embed the frame content size
     in the header, so we cannot use [Zstd.Decompressor.decompress_string]
  ...
  let written = Zstd.Decompressor.decompress_string_into_bytes compressed out in
...
let test_incremental_writes () =
  ...
  let push bs pos len =
    Buffer.add_string buf (Bstr.sub ~off:pos ~len bs |> Bstr.to_string)
  in
  let state = Zstd.Compressor.State.create ~level:1 () in
  let bs = Bstr.of_string data in
  ...
  for i = 0 to Bstr.length bs - 1 do
    Zstd.Compressor.State.feed ~push state bs i (i + 1) `Continue
  done;
  Zstd.Compressor.State.finish ~push state;
...
let test_flush () =
  ...
  let bs = Bstr.of_string data in
  Zstd.Compressor.State.feed ~push state bs 0 (Bstr.length bs) `Continue;
  Zstd.Compressor.State.feed ~push state Bstr.empty 0 0 `Flush;
...
let test_truncated_stream () =
  ...
  with
  | Zstd.Decompressor.State.Truncated_input -> ()
  | Failure _ -> ()
...
let test_gc_stress () =
  ...
  while !pos < Bstr.length bs do
    ...
    Zstd.Compressor.State.feed ~push state bs !pos next `Continue;
    gc_churn ();
    pos := next
  done;
  Zstd.Compressor.State.finish ~push state;
...
let test_dictionary () =
  ...
  let data = String.init 10_000 (fun i -> Char.chr (i mod 256)) in
  let compressed =
    Zstd.Compressor.compress_string ~dictionary:dict ~level:3 data
  in
  let decompressed =
    Zstd.Decompressor.decompress_string ~dictionary:dict compressed
  in
  Alcotest.(check string) "dictionary" data decompressed

let test_channels () =
  let data = String.init 10_000 (fun i -> Char.chr (i mod 256)) in
  let tmp_in = Filename.temp_file "zstd_test_in" ".txt" in
  let tmp_compressed = Filename.temp_file "zstd_test_compressed" ".zst" in
  let tmp_out = Filename.temp_file "zstd_test_out" ".txt" in
  Fun.protect
    ~finally:(fun () ->
      Sys.remove tmp_in;
...
let test_frame_size () =
  let data = String.init 10_000 (fun i -> Char.chr (i mod 256)) in
  let compressed = Zstd.Compressor.compress_string ~level:3 data in
  let size = Zstd.Compressor.Frame.uncompressed_size (`String compressed) in
...
let test_frame_size_bigstring () =
  ...
  in
  let compressed = Zstd.Compressor.compress_bigstring ~level:3 data in
  let size = Zstd.Compressor.Frame.uncompressed_size (`Bigstring compressed) in
...
let test_compress_into () =
  let data = "Hello, Zstandard!" in
  let bound = Zstd.Bindings_intf.compress_bound (String.length data) in
  let out = Bytes.create bound in
  let written = Zstd.Compressor.compress_string_into ~level:3 data out in
...
let test_already_closed () =
  ...
  with Zstd.Decompressor.State.Already_closed -> ()
